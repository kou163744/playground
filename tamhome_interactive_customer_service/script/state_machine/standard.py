...
from interactive_customer_service.msg import RobotStatus
...
class Wait4Start(smach.State, Logger):
    def __init__(self, outcomes):
        ...
        self.pub_to_customer = rospy.Publisher("/interactive_customer_service/message/robot", Conversation, queue_size=5)
    ...
    def wait_for_ready_msg(self, ready_msg="Are_you_ready?") -> None:
        ...
        is_wait_ready = rospy.get_param("/interactive_customer_service/wait_to_ready", True)
        while not rospy.is_shutdown():
            if is_wait_ready is False:
                break
            msg = rospy.wait_for_message("/interactive_customer_service/message/customer", Conversation, timeout=None)
            self.loginfo(msg.type)
            if msg.type == ready_msg:
                self.logsuccess(f"Start task {ready_msg}")
                pub_msg = Conversation()
                pub_msg.type = "I_am_ready"
                self.pub_to_customer.publish(pub_msg)
                break
        return
...
class Finish(smach.State, Logger):
    def __init__(self, outcomes):
        smach.State.__init__(self, outcomes=outcomes)
        Logger.__init__(self, loglevel="INFO")
        self.pub_to_moderator = rospy.Publisher("/interactive_cleanup/message/robot", Conversation, queue_size=5)

    def execute(self, userdata):
        self.loginfo("Finished")
        msg = Conversation()
        msg.type = "Task_finished"
