...
def call_vlm(api_key, image_path, order):
    ...
    text_prompt = f"You are given an image with various objects and their corresponding labels. The labels are represented as numbers placed above the objects. Your task is to identify the number based on a given instruction. Output only the number corresponding to the object that matches the instruction, with no additional text.\nInstruction: {order}\nNumber: "
...
def main(args: argparse.Namespace):
    ...
    image_path_base = '../io/YOLO/level2'
    csv_path = '../io/results/level2_results.csv'
    ...
    for col_q, col_an in zip(specific_columns, answer_columns):
        ...
        if col_q in csv_data.columns and col_an in csv_data.columns:
            ...
            for order, graund_truth in zip(csv_data[col_q], csv_data[col_an]):
                count += 1
                image_path = os.path.join(image_path_base, f"rgb_result{count}.jpg")
                # image_path = os.path.join(image_path_base, f"image{count}.png") # for som
                if isinstance(order, str):
                    responce = call_vlm(api_key, image_path, order)
            ...
            csv_data[f"{col_q}_ans"] = answer
            answer_rate = []
            answer_rate.append(f"{correct_count}")
            answer_rate.append(f"{float(correct_count // count)}")
            while len(answer_rate) != len(csv_data[f"{col_q}_ans"]):
                answer_rate.append(None)
            csv_data[f"{col_q}_ans_rate"] = answer_rate

    csv_data.to_csv('../io/results/updated_level2_results.csv')
